...
// 토스 파서는 표준 인터페이스를 그대로 사용

export class TossParser {
    /**
     * 토스 엑셀 파일을 표준 형식으로 변환
     * @param file 토스 엑셀 파일
    ...
    static async parse(file: File): Promise<StandardOrderData[]> {
        try {
            ...
            throw new Error(`토스 파일 파싱 실패: ${error}`);
        }
    }
    ...
    private static detectHeaderRow(sheet: XLSX.WorkSheet): number {
        ...
        // 기본값은 2행 (0-based index 1)
        return 1;
    }

    /**
     * 토스 행 데이터를 표준 데이터로 변환
     * @param row 토스 엑셀 행 데이터
     * @returns 표준화된 주문 데이터
     */
    private static mapRowToStandardData(
        ...
        return {
            ...
            order_date: FileUtils.parseTossDate(row["주문일자"]),
            receiver_name: String(row["수령인명"] || ""),
            receiver_phone: FileUtils.formatPhone(row["수령인 연락처"]),
            ...
            final_price: FileUtils.parsePrice(row["거래금액"]),
            platform: "토스",
            order_phone: FileUtils.formatPhone(row["구매자 연락처"]),
        };
    }
